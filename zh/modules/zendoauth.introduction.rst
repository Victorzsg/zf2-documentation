...
This authorization process is token based. If you authorize an application (and by application we can include any
...
problem of accessing private data via any web service *API*.

OAuth is not a completely new idea, rather it is a standardized protocol building on the existing properties of
protocols such as Google AuthSub, Yahoo BBAuth, Flickr *API*, etc. These all to some extent operate on the basis of
...
OAuth as an alternative and future replacement for their own solutions.
